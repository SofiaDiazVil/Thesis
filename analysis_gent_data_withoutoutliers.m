...
filename_hr = 'D:\Users\PERSONAL\Documents\psychologie Master thesis\experiment_gent\Group.mat';
...
%resting_hr  = hr_all(group == 1); %empty
erotic_hr   = hr_all(group == 2);
neutral_hr  = hr_all(group == 3);
% Seprate HR by condition when starting from the processed dataset
%erotic_hr   = hr_all(1:30);
%neutral_hr  = hr_all(31:60);
...
%resting_hrv = hrv_all(group == 1,:)'; %empty
erotic_hrv  = hrv_all(group == 2,:)';
neutral_hrv = hrv_all(group == 3,:)';
% Seprate HR by condition when starting from the processed dataset
%erotic_hrv   = hrv_all(1:30,:);
%neutral_hrv  = hrv_all(31:60,:);
...
% Save the SD of the HRV as a matrix 
sd_hrv_all = std(hrv_all,0,2);
filename_ISC = 'D:\Users\PERSONAL\Documents\psychologie Master thesis\experiment_gent\SD_HRV_ALL.mat';
save(filename_ISC,'sd_hrv_all');
%% Statistics ISC 
...
%resting_ISC  = ISC_all(61:90); %resting is not being processed 
%erotic_ISC   = ISC_all(1:30);
%neutral_ISC  = ISC_all(31:60);
...
for iRandShift=Npermutations:-1:1 
    ...
    ISC_partial_shifted = [];
      ...
      for i = 2:3    
        ...
        % Calculate the average correlation per participant, autocorrelation is not included
        ISC = tanh(nanmean(tmp_corr_mat));
        ISC_partial_shifted = [ISC_partial_shifted ISC]; % Stores erotic and then neutral
...
% Separte the data by condition 
erotic_ISC_shifted  = ISC_all_shifted(1:30,:);
neutral_ISC_shifted = ISC_all_shifted(31:60,:);
...
% distribution and compare your values to that 
    % Erotic
erotic_P95 = prctile(erotic_ISC_shifted,95,2);
erotic_P5  = prctile(erotic_ISC_shifted,5,2);
    % Neutral 
neutral_P95 = prctile(neutral_ISC_shifted,95,2);
neutral_P5  = prctile(neutral_ISC_shifted,5,2);   
...
pval_pos_erotic  = pval_pos(1:30)
pval_neg_erotic  = pval_neg(1:30)
pval_pos_neutral = pval_pos(31:60)
pval_neg_neutral = pval_neg(31:60)

%%
% Graph an example of the distribution particpant 10
% Select the data
