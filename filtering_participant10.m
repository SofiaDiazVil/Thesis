%%%
% this code is meant to filter the data for participant 10 (used only in 
% the first session data) it is based on the l1tf functions which use 
% detrending. The filtered file are saved, the unfiltered files are also 
% saved but labeled as raw in a separate folder 
%%%
%% particpant 10 
ECG = struct2array(load('ecg_sub10_resting_ses-20210707_raw.mat'));
x = double(ECG')
reg_par=0.0001;
lambda_max = l1tf_lambdamax(x);
[z1,status] = l1tf(x, reg_par*lambda_max);
ECGd=x-z1;plot(ECG,'DisplayName','ECG');
hold on;
plot(ECGd,'DisplayName','ECGd');
hold off;

save("ecg_sub10_resting_ses-20210707.mat","ECGd")



%% Participant 14 
file = 'neutral_ecg_subj14.mat'
ECG = double(struct2array(load(file)));
ECG_inv = ECG*-1;

save(file,"ECG_inv")

file = 'erotic_ecg_subj14.mat'
ECG = double(struct2array(load(file)));
ECG_inv = ECG*-1;
...
%% Patient 10 
file = 'ecg_sub10_neutro_ses-20210707';
ECG = double(struct2array(load(file)));
ECG_inv = ECG*-1;
